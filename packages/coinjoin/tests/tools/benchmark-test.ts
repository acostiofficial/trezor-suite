...
const [fromHash, batchSizeString = '500'] = process.argv.slice(2);
const batchSize = Number(batchSizeString);

const agent = undefined && new SocksProxyAgent({ host: TOR_HOST, port: TOR_PORT });
    ...
    return (bestKnownBlockHash: string) => {
        ...
        return new Promise<string[]>((resolve, reject) => {
            ...
            ws.once('message', (message: string) => {
                ...
                const { id, data } = resp;
                if (reqID !== id || data.error) {
                    reject(new Error(`Error ${id} ${reqID} ${data.error}`));
                } else {
                    resolve(data.map((row: string) => row.split(':')[1]));
                }
            });
            ws.send(
                JSON.stringify({
                    ...
                    params: { bestKnownBlockHash, pageSize: batchSize },
                }),
            );
        });
    };
...
(async () => {
    ...
    let batch = await filtersFromWasabi(fromHash);
    ...
    batch = await filtersFromBlockbook(fromHash);
    while (batch.length === batchSize + 1) {
        // eslint-disable-next-line no-await-in-loop
